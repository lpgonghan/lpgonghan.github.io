...
### 总结
了解了属性编码类型的只是，再回过头来看 ```YYClassInfo``` 这个类，是不是就不会觉得上来就是一堆看不懂的东西，让人无从下手了呢？至于类中的其余子类和方法，网上找一篇 runtime 相关的文章，基本都会涉及到，或者直接撸[源码](https://opensource.apple.com/tarballs/objc4/)这里就不再赘述了。  
